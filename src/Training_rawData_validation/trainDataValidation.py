...
class trainDataValidationClass:
    ...
    """This is a custom class for training data validation"""

    def __init__(self):

        self.config=read_params('params.yaml')
        self.schema_path=os.path.join(self.config["schema"]["dir"],self.config["schema"]["training"])
    ...
    def getvaluesfromSchema(self,file):
        ...
        try:
            ...
            self.logger.log(self.file,f"getvaluesfromSchema : ValueError:Value not found inside schema_training.json")
            self.file.close()
            raise ValueError
    ...
    def createGoodBadrawDataDirectory(self,file):
        ...
        self.file=file
        try:
            path = os.path.join(self.config["directories"]["validatedFiles"]["training"], self.config["directories"]["rawGood"])
            os.makedirs(path,exist_ok=True)
            path = os.path.join(self.config["directories"]["validatedFiles"]["training"], self.config["directories"]["rawBad"])
            os.makedirs(path,exist_ok=True)
            self.logger.log(self.file,f"Good and Bad Directories created")

        except OSError as ex:
            self.logger.log(self.file,f"Error while creating Directory {ex}")
            raise OSError
    ...
    def deleteGoodBadrawDataDirectory(self,file):
        ...
        self.file=file
        try:
            main_dir=self.config["directories"]["validatedFiles"]["training"]
    ...
    def validateFiles(self,regex,timeStampLength,dateStampLength,NumberofColumns, batch_dir_path,file):
        ...
        self.goodFiledir=os.path.join(self.config["directories"]["validatedFiles"]["training"], self.config["directories"]["rawGood"])
        self.badFiledir=os.path.join(self.config["directories"]["validatedFiles"]["training"], self.config["directories"]["rawBad"])
